...
const _APPS = preload("services/apps.gd")
...
const _OAUTH2 = preload("services/oauth2.gd")
...
var apps : _APPS = _APPS.new(_client) ## No description is provided
...
var oauth2 : _OAUTH2 = _OAUTH2.new(_client) ## The OAuth2 service allows you to authorize apps and issue standards-based OAuth2 and OpenID Connect tokens.
var organization : _ORGANIZATION = _ORGANIZATION.new(_client) ## The Organization service allows you to manage organization-level projects.
...
const RUNTIME = preload("enums/runtime.gd")
...
const STATUSCODE = preload("enums/status_code.gd")
const PROXYRESOURCETYPE = preload("enums/proxy_resource_type.gd")
const FRAMEWORK = preload("enums/framework.gd")
const BUILDRUNTIME = preload("enums/build_runtime.gd")
const ADAPTER = preload("enums/adapter.gd")
...
func set_forwarded_user_agent(value: String) -> Appwrite:
    _client.set_forwarded_user_agent(value)
    return self
