class_name AppwriteAttributeEnum
extends RefCounted
## AttributeEnum[br]
...
var elements: Array ## Array of elements in enumerated type.
...
static func from_dict(dict: Dictionary):
    var m := AppwriteAttributeEnum.new()
    ...
    for key in _FIELD_MAP:
        ...
        if key == "status" and value != null:
            ...
            m.set(key, value)
            continue
        if key == "elements" and value is Array:
            m.set(key, value)
            continue
        if value != null:
            m.set(key, value)
